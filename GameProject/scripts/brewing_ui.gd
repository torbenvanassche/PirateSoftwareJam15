...
func on_item_added():
	...
	visual_element.get_node("Button").pressed.connect(_remove_item.bind(visual_element, added_slot.slot_data.item));
	...
	var brew = get_brew()
	if brew != {} && brew.has("brew_color"):
		Manager.instance.campfire.recolor_fluid(brew.brew_color)
...
func remove_all():
	for i in added_items.get_children():
		i.get_node("Button").pressed.emit();
	Manager.instance.campfire.reset_color()
	
func get_brew():
	var item_names = component_array.map(func(x): return x.id);
	return ItemManager.find_item_with_components(item_names);
	
func _on_brew():
	var brew = get_brew();
	if brew != {}:
		Manager.instance.player.inventory.add_item(brew);
	remove_all();
